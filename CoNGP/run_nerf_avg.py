...
def create_nerf(args):
    ...
    embed_fn_1, input_ch_1 = get_embedder(args.multires, args, i=args.i_embed)
    
    if args.i_embed==1:
        # hashed embedding table
        embedding_params = list(embed_fn.parameters())
        embedding_params_1 = list(embed_fn_1.parameters())
    ...
    if args.use_viewdirs:
        ...
        embeddirs_fn_1, input_ch_views_1 = get_embedder(args.multires_views, args, i=args.i_embed_views)
    ...
    if args.i_embed==1:
        ...
        model_1 = NeRFSmall(num_layers=2,
                        hidden_dim=64,
                        geo_feat_dim=15,
                        num_layers_color=3,
                        hidden_dim_color=64,
                        input_ch=input_ch, input_ch_views=input_ch_views_1).to(device)
    else:
        model = NeRF(D=args.netdepth, W=args.netwidth,
                 input_ch=input_ch, output_ch=output_ch, skips=skips,
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
    grad_vars = list(model_1.parameters())
    ...
    if args.N_importance > 0:
        if args.i_embed==1:
            model_fine = NeRFSmall(num_layers=2,
                        ...
                        hidden_dim_color=64,
                        input_ch=input_ch, input_ch_views=input_ch_views).to(device)
            model_fine_1 = NeRFSmall(num_layers=2,
                        hidden_dim=64,
                        geo_feat_dim=15,
                        num_layers_color=3,
                        hidden_dim_color=64,
            ...
            model_fine = NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
                          input_ch=input_ch, output_ch=output_ch, skips=skips,
                          input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
        grad_vars += list(model_fine_1.parameters())

    network_query_fn = lambda inputs, viewdirs, network_fn : run_network(inputs, viewdirs, network_fn,
                                                                embed_fn=embed_fn,
                                                                embeddirs_fn=embeddirs_fn,
                                                                netchunk=args.netchunk)
    network_query_fn_1 = lambda inputs, viewdirs, network_fn_1 : run_network(inputs, viewdirs, network_fn_1,
                                                                embed_fn=embed_fn_1,
                                                                embeddirs_fn=embeddirs_fn_1,
                                                                netchunk=args.netchunk)
    ...
    render_kwargs_train = {
        ...
        'network_query_fn_1' : network_query_fn_1,
        'perturb' : args.perturb,
        'N_importance' : args.N_importance,
        'network_fine' : model_fine,
        'network_fine_1' : model_fine_1,
        'N_samples' : args.N_samples,
        'network_fn' : model,
        'network_fn_1' : model_1,
        'embed_fn': embed_fn,
        'embed_fn_1': embed_fn_1,
...
def render_rays(ray_batch,
    ...
    raw_1 = network_query_fn(pts, viewdirs, network_fn)
    raw_2 = network_query_fn_1(pts, viewdirs, network_fn_1)

    raw = raw_1.detach() + raw_2
    ...
    if N_importance > 0:
        ...
        run_fn_1 = network_fn if network_fine_1 is None else network_fine_1
#         raw = run_network(pts, fn=run_fn)
        
        raw_1 = network_query_fn(pts, viewdirs, run_fn)
        raw_2 = network_query_fn_1(pts, viewdirs, run_fn_1)

        raw = raw_1.detach() + raw_2

        '''
        rgb_map_1, disp_map_1, acc_map_1, weights_1, depth_map_1, sparsity_loss_1 = raw2outputs(raw_1, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
        rgb_map_2, disp_map_2, acc_map_2, weights_2, depth_map_2, sparsity_loss_2 = raw2outputs(raw_2, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)
        rgb_map_3, disp_map_3, acc_map_3, weights_3, depth_map_3, sparsity_loss_3 = raw2outputs(raw_1, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)

        rgb_map = (rgb_map_1 + rgb_map_2 + rgb_map_3) / 3.
        '''
