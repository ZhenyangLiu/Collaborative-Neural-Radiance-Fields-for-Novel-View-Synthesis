...
class TensorVMSplit(TensorBase):
    ...
    def init_svd_volume(self, res, device):
        ...
        self.density_plane_res, self.density_line_res = self.init_one_svd(self.density_n_comp, self.gridSize, 0.1, device)
        
        self.app_plane, self.app_line = self.init_one_svd(self.app_n_comp, self.gridSize, 0.1, device)
        self.app_plane_res, self.app_line_res = self.init_one_svd(self.app_n_comp, self.gridSize, 0.1, device)
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [{'params': self.density_line_res, 'lr': lr_init_spatialxyz}, {'params': self.density_plane_res, 'lr': lr_init_spatialxyz},
                     {'params': self.app_line_res, 'lr': lr_init_spatialxyz}, {'params': self.app_plane_res, 'lr': lr_init_spatialxyz},
                         {'params': self.basis_mat.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
    ...
    def compute_densityfeature(self, xyz_sampled):
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            plane_coef_point_res = F.grid_sample(self.density_plane_res[idx_plane], coordinate_plane[[idx_plane]],align_corners=True).view(-1, *xyz_sampled.shape[:1])
            ...
            line_coef_point_res = F.grid_sample(self.density_line_res[idx_plane], coordinate_line[[idx_plane]],align_corners=True).view(-1, *xyz_sampled.shape[:1])

            sigma_feature = sigma_feature + torch.sum(plane_coef_point * line_coef_point, dim=0) + torch.sum(plane_coef_point_res * line_coef_point_res, dim=0)
    ...
    def compute_appfeature(self, xyz_sampled):
        ...
        plane_coef_point_res,line_coef_point_res = [],[]
        for idx_plane in range(len(self.app_plane)):
            plane_coef_point.append(F.grid_sample(self.app_plane[idx_plane], coordinate_plane[[idx_plane]],
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1]))
            plane_coef_point_res.append(F.grid_sample(self.app_plane_res[idx_plane], coordinate_plane[[idx_plane]],
                                                align_corners=True).view(-1, *xyz_sampled.shape[:1]))
            
            line_coef_point.append(F.grid_sample(self.app_line[idx_plane], coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1]))
            line_coef_point_res.append(F.grid_sample(self.app_line_res[idx_plane], coordinate_line[[idx_plane]],
                                            align_corners=True).view(-1, *xyz_sampled.shape[:1]))
        
        plane_coef_point, line_coef_point = torch.cat(plane_coef_point), torch.cat(line_coef_point)
        plane_coef_point_res, line_coef_point_res = torch.cat(plane_coef_point_res), torch.cat(line_coef_point_res)


        return self.basis_mat((plane_coef_point * line_coef_point).T + (plane_coef_point_res * line_coef_point_res).T)
